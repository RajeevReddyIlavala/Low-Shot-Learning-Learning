...
def training_loop(lowshot_dataset, num_classes, params, batchsize=1000, maxiters=1000, index=0, test_file=None, basecl=None, novelcl=None, lowshotn=None, exp_id=None):
    ...
    for i in range(maxiters):
        ...
        loss.backward()
        optimizer.step()
        # torch.save(deepcopy(model).state_dict(), "/scratch/pa1371/low-shot-shrink-hallucinate/models/models_saved/model_lowshotn_" + str(lowshotn) + "_expid_" + str(exp_id) + '_iter_' + str(i))
	# with h5py.File(test_file, 'r') as f:
	#     test_loader = get_test_loader(f)
	#     accu_values.append(eval_loop(test_loader,model, basecl, novelcl))
        if (i%100==0):
            print('{:d}: {:f}'.format(i, loss.data[0]))
    return model,accu_values 
...
if __name__ == '__main__':
    ...
    with h5py.File(params.trainfile, 'r') as f:
        ...
        model, act_accu_values = training_loop(lowshot_dataset, params.numclasses, params, params.batchsize, params.maxiters, index=params.model_index, test_file=params.testfile, basecl=base_classes, novelcl=novel_classes, lowshotn=params.lowshotn, exp_id=params.experimentid)

    act_accu_values = [] #np.vstack(act_accu_values).tolist()
    with h5py.File(params.testfile, 'r') as f:
        test_loader = get_test_loader(f)
    ...
    with open(outpath, 'w') as f:
        json.dump(dict(lr=params.lr,wd=params.wd, expid=params.experimentid, lowshotn=params.lowshotn, accs=accs.tolist(), act_accu_values=act_accu_values),f)
